...
{
    internal async Task<MontaRestApiModels_Dto_OrderDto> GetOrderWebshoporderidAsync(GetOrderWebshoporderidRequest req, System.Threading.CancellationToken cancellationToken = default)
    {
        try
        {
            var path = "/order/{webshoporderid}";
            ...
            if (req.Channel is not null) q["channel"] = Convert.ToString(req.Channel, System.Globalization.CultureInfo.InvariantCulture);
            ...
            return await ReadJsonAsync<MontaRestApiModels_Dto_OrderDto>(resp).ConfigureAwait(false);
        }

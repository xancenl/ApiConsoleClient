...
{
    internal async Task<MontaRESTApiModels_Orders_OrderDeliveryInformation> GetOrderWebshoporderidExpectedprocessingtimeframeAsync(GetOrderWebshoporderidExpectedprocessingtimeframeRequest req, System.Threading.CancellationToken cancellationToken = default)
    {
        try
        {
            var path = "/order/{webshoporderid}/expectedprocessingtimeframe";
            ...
            return await ReadJsonAsync<MontaRESTApiModels_Orders_OrderDeliveryInformation>(resp).ConfigureAwait(false);
        }

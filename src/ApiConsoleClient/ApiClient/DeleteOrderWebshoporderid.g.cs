...
{
    internal async Task DeleteOrderWebshoporderidAsync(DeleteOrderWebshoporderidRequest req, System.Threading.CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var resp = await flreq.SendJsonAsync(new HttpMethod("DELETE"), req.Body!, cancellationToken: cancellationToken).ConfigureAwait(false);
            return;
        }

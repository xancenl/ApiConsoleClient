...
{
    internal async Task<string> PutReturnIdUpdatereturnstatusStatusAsync(PutReturnIdUpdatereturnstatusStatusRequest req, System.Threading.CancellationToken cancellationToken = default)
    {
        try
        {
            var path = "/return/{id}/update_return_status/{status}";
            ...
            path = path.Replace("{Id}", __val_Id);
            var __val_Status = Convert.ToString(req.Status, System.Globalization.CultureInfo.InvariantCulture) ?? string.Empty;
            path = path.Replace("{status}", __val_Status);
            path = path.Replace("{Status}", __val_Status);
            ...
            var resp = await flreq.SendAsync(new HttpMethod("PUT"), null, completionOption: System.Net.Http.HttpCompletionOption.ResponseContentRead, cancellationToken: cancellationToken).ConfigureAwait(false);

...
{
    internal async Task<MontaRestApiModels_Dto_ProductsDto> GetProductsAsync(GetProductsRequest req, System.Threading.CancellationToken cancellationToken = default)
    {
        try
        {
            var path = "/products";
            ...
            if (req.IncludeSplitStock is not null) q["includeSplitStock"] = (Convert.ToString(req.IncludeSplitStock) ?? string.Empty).ToLowerInvariant();

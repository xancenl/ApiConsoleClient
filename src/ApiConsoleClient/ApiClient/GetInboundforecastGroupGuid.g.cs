...
{
    internal async Task<MontaRESTApiModels_InboundForecast_InboundForecastGroup> GetInboundforecastGroupGuidAsync(GetInboundforecastGroupGuidRequest req, System.Threading.CancellationToken cancellationToken = default)
    {
        try
        {
            var path = "/inboundforecast/group/{Guid}";
            var __val_Guid = Convert.ToString(req.Guid, System.Globalization.CultureInfo.InvariantCulture) ?? string.Empty;
            path = path.Replace("{guid}", __val_Guid);
            path = path.Replace("{Guid}", __val_Guid);
